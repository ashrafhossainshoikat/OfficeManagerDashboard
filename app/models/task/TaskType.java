...
public class TaskType extends Model {
	...
	@Required
	//@Column(unique=true)  // Not unique, bsso or ccso tasks may have same name
	...
	public List<ValidationError> validate() {
		...
		TaskType o = find.where()
						.eq("taskName", this.taskName)
						.eq("isBssoTask", this.isBssoTask)
						.findUnique();
		
		if(this.id == null || (o.id != this.id)) {
			if(o != null) {
				if (errors == null) { errors = new ArrayList<ValidationError>(); }
				errors.add(new ValidationError("taskName", "Task Name already exists"));
			}
		}
